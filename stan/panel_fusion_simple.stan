...
model {
  ...
  for (i in 1:N) {
    ...
    mu[i, 1] = alpha[1] + x_all[i, 1] * beta[1];
    mu[i, 2] = alpha[2] + std_small[i, 2] * beta[2];
     
    mu_x[i] = alpha_x + y_all[i] .* beta_x;
  }
  
  x_all ~ multi_normal_cholesky(mu_x,  L);
  y_all ~ multi_normal_cholesky(mu, diag_pre_multiply(sigma, L));
}
generated quantities {
   ...
   
   {
    ...
    for (i in 1:N) {
      y_all[i, 1] = y_big[i];
      y_all[i, 2] = y_small[i];
      mu[i, 1] = alpha[1] + x_all_raw[1, i] * beta[1];
      mu[i, 2] = alpha[2] + std_small[i, 2] * beta[2];
      mu_x[i] = alpha_x + y_all[i] .* beta_x;
      
    x_rep[i] = (multi_normal_cholesky_rng(mu_x[i], L) .* sd_hours + mean_hours)';
       while (x_rep[i, 1] < 0 || x_rep[i, 2] < 0 )
          x_rep[i] = (multi_normal_cholesky_rng(mu_x[i], L) .* sd_hours + mean_hours)';
      
      y_rep[i] = multi_normal_cholesky_rng(mu[i], diag_pre_multiply(sigma, L))' .* [400, 10];
